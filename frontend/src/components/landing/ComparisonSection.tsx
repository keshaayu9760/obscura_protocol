...
const comparisons = [
  { feature: 'Bet Privacy', obscura: 'Full ZK', polymarket: 'Public', azuro: 'Public' },
  { feature: 'Identity Hidden', obscura: 'Yes', polymarket: 'No', azuro: 'Pseudo' },
  { feature: 'Position Privacy', obscura: 'Encrypted Records', polymarket: 'On-chain', azuro: 'On-chain' },
  { feature: 'Front-Running', obscura: 'Impossible', polymarket: 'Possible', azuro: 'Possible' },
  { feature: 'Eclipse Rounds (24h–30d)', obscura: 'Yes', polymarket: 'No', azuro: 'No' },
  { feature: 'Private Payouts', obscura: 'Yes', polymarket: 'No', azuro: 'No' },
  { feature: 'On-Chain FPMM', obscura: 'Yes', polymarket: 'No', azuro: 'Yes' },
];
    ...
    <section className="py-28 px-4">
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
          <h2 className="section-title mb-4">Operational Edge</h2>
          <p className="text-smoke/60 max-w-xl mx-auto">See how Obscura Protocol compares to public-first prediction markets.</p>
        </motion.div>
              ...
              <thead>
                <tr className="border-b border-white/[0.04]">
                  ...
                  <th className="px-6 py-5 text-center text-[10px] font-heading font-semibold text-teal uppercase tracking-wider">
                    <span className="flex items-center justify-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-teal animate-pulse" />Obscura Protocol</span>
                  </th>
              ...
              <tbody>
                {comparisons.map((row, i) => (
                  <motion.tr key={row.feature} className="border-b border-white/[0.03] last:border-0 hover:bg-teal/[0.02] transition-colors"
                    ...
                    <td className="px-6 py-4 text-center text-sm font-medium text-teal"><span className="text-accent-green text-xs mr-1">✓</span>{row.obscura}</td>
